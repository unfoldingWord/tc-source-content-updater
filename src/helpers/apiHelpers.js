...
async function searchSubjects(subjects, owner, retries=3) {
  ...
  let fetchUrl = `https://qa.door43.org/api/catalog/v3/search?subject=${subjectParam}`;
  if (owner) {
...
export async function searchCatalogNext(searchParams, retries=3) {
  ...
  try {
    let fetchUrl = `https://qa.door43.org/api/catalog/v5/search`;
    let parameters = '';
...
export async function downloadManifestData(owner, repo, tag = 'master', retries=5) {
  const fetchUrl = `https://qa.door43.org/api/catalog/v5/entry/${owner}/${repo}/${tag}/metadata`;
  try {
...
export async function getLatestRelease(owner, repo, retries=5, stage = null) {
  let fetchUrl = `https://qa.door43.org/api/catalog/v5/search/${owner}/${repo}`;
  if (stage) {
...
export async function getReleaseMetaData(owner, repo, tag, retries=5) {
  const fetchUrl = `https://qa.door43.org/api/catalog/v5/entry/${owner}/${repo}/${tag}`;
  try {
    const {result} = await makeJsonRequestDetailed(fetchUrl, retries);
    return result;
  } catch (e) {
